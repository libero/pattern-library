...
const buildConfig = (invocationArgs, publicRoot, sourceRoot, testRoot, exportRoot, buildRoot) => {
  ...
  config.dir.build.js = `${config.buildRoot}/js`;
  ...
  config.dir.public.js = `${config.publicRoot}/js`;
  ...
  config.files.build.js = [
    `${config.dir.build.js}/**/*.js`,
    `${config.dir.build.js}/**/*.js.map`,
  ];
...
const cleanJs = () => del(config.files.build.js);
...
export const build = gulp.parallel(buildCss, buildJs, buildLinks);
...
const exportJs = () =>
  gulp.src(config.files.build.js)
...
export const watch = gulp.parallel(watchJs, watchLinks, watchSass, watchSharedConfig);
